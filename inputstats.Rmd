...
ggplot() +
  ...
  geom_point(data = subset(pop,Deme=="pure"), aes(x=long, y = lat), size=1) +
...
ggsave("~/Downloads/grayfox_map_ew.png", width=7, height = 3, units="in")


ggplot() +
  geom_polygon(data = usa, aes(x=long, y = lat, group=group), 
               fill = "white", color="gray70", linewidth=0.3) + 
  geom_point(data = pop, aes(x=long, y = lat, color = State), size=1) +
  coord_fixed(xlim = c(-123, -75),  ylim = c(26, 41.8)) +
  #scale_fill_manual(values = c("dodgerblue2", "goldenrod", "red2"))+ 
...
ggsave("~/Downloads/grayfox_map.png", width=7, height = 3, units="in")
...
eastpi <- read_tsv("inputstats_files/east6_pi_10kb.out.windowed.pi") %>%
  mutate(chr_position = ((BIN_START + BIN_END)/2)/1000000) %>%
  mutate(CHROM=as.numeric(str_remove(CHROM, "chr"))) 

westpi <- read_tsv("inputstats_files/west6_pi_10kb.out.windowed.pi") %>%
  mutate(chr_position = ((BIN_START + BIN_END)/2)/1000000) %>%
  mutate(CHROM=as.numeric(str_remove(CHROM, "chr"))) 
...
cfeastpi <- read_tsv("inputstats_files/cf_east6_pi_10kb.out.windowed.pi") %>%
  mutate(chr_position = ((BIN_START + BIN_END)/2)/1000000) %>%
  mutate(CHROM=as.numeric(str_remove(CHROM, "chr"))) 

cfwestpi <- read_tsv("inputstats_files/cf_west6_pi_10kb.out.windowed.pi") %>%
  mutate(chr_position = ((BIN_START + BIN_END)/2)/1000000) %>%
  mutate(CHROM=as.numeric(str_remove(CHROM, "chr"))) 
...
het <- read_tsv("inputstats_files/het.het") 
...
snps <- read_tsv("inputstats_files/snps.txt")
