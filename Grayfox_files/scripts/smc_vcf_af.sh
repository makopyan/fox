#!/bin/sh
#SBATCH --job-name=smc_vcf_af
#SBATCH --output=smc_vcf_af_%a.out
#SBATCH --error=smc_vcf_af_%a.err
#SBATCH --array=1-24
...
VCF_FILE="arcticfox_filtered.renameChroms.Mainland.drop295.QUAL30_DPgr205lt500_ANgr59.vcf.gz"
BED_FILE="arcticfox.rep.map.genic.bed.gz"
...
for SAMPLE in "${WES_SAMPLES[@]}"; do
    ...
    smc++ vcf2smc \
    -d $SAMPLE $SAMPLE -m $MAP_FILE $VCF_FILE wes_${SAMPLE}_chr${SLURM_ARRAY_TASK_ID}.af.smc.gz \
...
for SAMPLE in "${EAS_SAMPLES[@]}"; do
    ...
    smc++ vcf2smc \
    -d $SAMPLE $SAMPLE -m $MAP_FILE $VCF_FILE eas_${SAMPLE}_chr${SLURM_ARRAY_TASK_ID}.af.smc.gz \
