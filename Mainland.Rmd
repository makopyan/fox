...
pop <- read_tsv("mainland/mainland_WGSmetadata.txt") %>% 
...
ggsave("mainland/samples.png", width=6.5, height = 2.5, units="in")
...
#Grayfox West
gfplotwest <- read_csv("mainland/gf_west_plot.csv")
#Grayfox East
gfploteast <- read_csv("mainland/gf_east_plot.csv")
#Combine Grayfox
gfplot<- bind_rows(gfplotwest,gfploteast)

#Canfam4 West
cf4plotwest <- read_csv("mainland/cf4_west_plot.csv")
#Canfam4 East
cf4ploteast <- read_csv("mainland/cf4_east_plot.csv")
#Combine Canfam4
cf4plot<- bind_rows(cf4plotwest,cf4ploteast) 

#Canfam3 West
cf3plotwest <- read_csv("mainland/cf3_west_plot.csv")
#Canfam3 East
cf3ploteast <- read_csv("mainland/cf3_east_plot.csv")
#Combine Canfam3
...
pcfgf <- cfgf %>% 
  ...
  mutate(Ne=plyr::round_any(Ne,1000),start=plyr::round_any(start,1000),end=plyr::round_any(end,1000)) %>% ungroup()


pcf4e <- pcfgf %>% filter(genome=="Canfam4" & pop=="East") %>% select(genome,start,end,Ne) 
pcf3e <- pcfgf %>% filter(genome=="Canfam3" & pop=="East") %>% select(genome,start,end,Ne) 
pgfe <- pcfgf %>% filter(genome=="Grayfox" & pop=="East") %>% select(genome,start,end,Ne) 

pcf4w <- pcfgf %>% filter(genome=="Canfam4" & pop=="West") %>% select(genome,start,end,Ne) 
pcf3w <- pcfgf %>% filter(genome=="Canfam3" & pop=="West") %>% select(genome,start,end,Ne) 
pgfw <- pcfgf %>% filter(genome=="Grayfox" & pop=="West") %>% select(genome,start,end,Ne) 
...
synteny<-read_tsv("mainland/syn_chrom_mean.txt")
